<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WorkReminderList2.aspx.cs"
    MasterPageFile="~/SysManager/include/H-uiSecond.Master" Inherits="OAWebSite2016.SysManager.WebUI.WorkLogMng.WorkReminderList2" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <!--导航区设置-->
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 工作备忘 <span class="c-gray en">&gt;</span> 已处理备忘 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="pd-20">
        <!--查询区设置-->
        <div class="text-c">
            ...
            &nbsp;&nbsp; <span class="select-box" style="width: 120px">
                <jasp:JDropDownList ID="datatype" runat="server" AutoPostBack="true" CssClass="select">
                    <asp:ListItem Text="本人备忘" Value="0"></asp:ListItem>
                    <asp:ListItem Text="他人备忘" Value="1"></asp:ListItem>
                </jasp:JDropDownList>
            </span>&nbsp;&nbsp;
                ...
                <jasp:SecurityPanel ID="sec1" runat="server">
                    <a href="javascript:;" onclick="javascript:return NewData();" class="btn btn-primary radius">
                        <i class="Hui-iconfont">&#xe600;</i> 登记备忘</a> &nbsp;&nbsp; <a href="javascript:;"
                            onclick="javascript:return HasNoDoneData();" class="btn btn-primary radius"><i class="Hui-iconfont">
                                &#xe6a6;</i> 标为未处理</a>
                </jasp:SecurityPanel>
        ...
        <div style="display: none">
            ...
            <jasp:JTextBox ID="OtherMan" runat="server"></jasp:JTextBox>
        </div>
        <jasp:JDataSource ID="data1" runat="server" JType="Table" SqlID="Tb_WorkReminderView"
            ...
            <ParaItems>
                ...
                <jasp:ParameterItem ParaType="Control" ParaName="OtherMan" DataName="AuthorID" SearchOperator="NotEqual" />
                <jasp:ParameterItem ParaType="String" ParaName="xx" DataName="IsDone=true" SearchOperator="UserDefine" />
            </ParaItems>
        </jasp:JDataSource>
        <!--数据表格-->
                    ...
                    <asp:Repeater ID="Repeater1" runat="server" DataSourceID="data1" EnableViewState="false">
                        <ItemTemplate>
                            <tr class="text-c">
                                ...
                                <td class="operate">
                                    <a onclick="javascript:return EditLog('<%#Eval("LogID") %>');" title="点击查看日志" href='#'><%#KORWeb.BUL.JLogBU.GetFileCount(Eval("LogID").ToString())%></a>
                                </td>
                                <td>
                                    <%#Eval("JB") %>
                                </td>
    ...
    <script type="text/javascript">
        ...
        //4*--查看日志
        function EditLog(dataid) {
            var title = "相关日志列表";
            var url = "JLog_Detail.aspx?LogID=" + dataid;
            ...
            if (seldata == "") {
                ...
                layer.confirm('确认要将选中的数据标记为【已处理】吗？', function(index) {
                    $.ajax({
                        ...
                        data: {
                            KeyID: seldata,
                            Action: 'HasNoneDone_WorkReminder'
                        },
                        success: function(data) {
                            if ("ok" == data) {
                                layer.alert("【标为未处理】操作成功！", function() {
                                    $('#butSearch').click();  //重新加载页面
                                });
                            } else {
                                layer.alert("【标为未处理】操作失败！");
                            }
                        }
                    });
                });

...
{
    public partial class Tb_Project_Detail_ADD3 : System.Web.UI.Page
    {
        protected override void OnInit(EventArgs e)
        {
            this.btnSubmit.ServerClick += new EventHandler(btnSubmit_ServerClick);
            this.selecProject.SelectedIndexChanged += new EventHandler(selecProject_SelectedIndexChanged);
            base.OnInit(e);
        }

        void selecProject_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (this.selecProject.SelectedValue != "")
            {
                KORWeb.BUL.Tb_PlanBU bu1 = new KORWeb.BUL.Tb_PlanBU();
                int week1=bu1.GetProjectDefaultWeek(this.selecProject.SelectedValue);
                this.PlanWeek.Text = week1 + "";
            }
        }
        ...
        void btnSubmit_ServerClick(object sender, EventArgs e)
        {
            ...
            //判断项目的周次
            if (String.IsNullOrEmpty(this.PlanWeek.Text)
                || WebFrame.Util.JValidator.IsInt(this.PlanWeek.Text) == false)
            {
                KORWeb.BUL.UTools.Alert("错误：请输入一个项目的合适周次！");
                return;
            }
            ...
            List<KORWeb.DDL.Tb_PlanDetailDD> list1 = bu1.GetWeekPlanDetailListByExcel(this.file1, ref fname);
            if (list1.Count == 0)
            {
                KORWeb.BUL.UTools.Alert("错误：请选择一个合适的总体计划Excel文件！");
                return;
            ...
            bool succ = bu.ImportExcelToWeekPlan(this.selecProject.SelectedValue, int.Parse(this.PlanWeek.Text),
                DateTime.Parse(this.begDate.Text),DateTime.Parse(this.endDate.Text),
                this.Remark.Text, this.file1.FileName, fname, list1);
            if (succ)
            {

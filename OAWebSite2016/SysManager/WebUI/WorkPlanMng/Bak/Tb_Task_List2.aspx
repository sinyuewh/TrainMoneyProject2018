<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Tb_Task_List2.aspx.cs"
    MasterPageFile="~/SysManager/include/H-uiSecond.Master" Inherits="OAWebSite2016.SysManager.WebUI.WorkPlanMng.Tb_Task_List2" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <!--导航区设置-->
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 工单管理 <span class="c-gray en">&gt;</span> 待审核工单 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="pd-20">
        <!--查询区设置-->
                        ...
                        <ItemTemplate>
                            <tr class="text-c">
                                ...
                                <% if (this.IsManager)
                                   { %>
                                <td class="operate">
                                    <a onclick="javascript:return CompleteTask('<%#Eval("GuidID") %>');" href='#' title="将工单标为完成工单">
                                        <img alt="" src="/images/icon/office_001.png" width="16" height="16" border="0" />完成</a><br />
                                    <a onclick="javascript:return CompleteNoTask('<%#Eval("GuidID") %>');" href='#' title="将工单标为失败工单">
                                        <img alt="" src="/images/icon/office_005.png" width="16" height="16" border="0" />失败</a><br />
    ...
    <script type="text/javascript">
        ...
        function CompleteTask(dataid) {
            layer.confirm("提示：确定要将工单标记为【完成】吗？", function() {
                $.ajax({
                    type: 'POST',
                    url: "Handler/DoAction.aspx",
                    data: {
                    ...
                    success: function(data) {
                        if ("ok" == data) {
                            layer.alert("将工单标记为【完成】操作成功！", function() {
                                $('#butSearch').click();  //重新加载页面
                            });
                        } else {
                            layer.alert("将工单标记为【完成】操作失败！");
                        }
                    }
                });
            });
        ...
        function CompleteNoTask(dataid) {
            layer.confirm("提示：确定要将工单标记为【失败】吗？", function() {
                $.ajax({
                    type: 'POST',
                    url: "Handler/DoAction.aspx",
                    data: {
                    ...
                    success: function(data) {
                        if ("ok" == data) {
                            layer.alert("将工单标记为【失败】操作成功！", function() {
                                $('#butSearch').click();  //重新加载页面
                            });
                        } else {
                            layer.alert("将工单标记为【失败】操作失败！");
                        }
                    }
                });
            });

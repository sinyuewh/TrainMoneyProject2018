<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Tb_Task_List1.aspx.cs"
    MasterPageFile="~/SysManager/include/H-uiSecond.Master" Inherits="OAWebSite2016.SysManager.WebUI.WorkPlanMng.Tb_Task_List1" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <!--导航区设置-->
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 工单管理 <span class="c-gray en">&gt;</span> 完成中工单 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="pd-20">
        <!--查询区设置-->
        <div class="text-c">
        ...
        <div style="display: none">
            <jasp:JTextBox ID="ExecuteManID" runat="server"></jasp:JTextBox>
        </div>
        <jasp:JDataSource ID="data1" runat="server" JType="Table" SqlID="Tb_Task" PageSize="-1"
            ...
            <ParaItems>
                ...
                <jasp:ParameterItem ParaType="Control" ParaName="Time1" DataName="AddTime<=cdate('{0} 23:59:59')"
                    SearchOperator="UserDefine" />
                <jasp:ParameterItem ParaType="String" ParaName="xx" DataName="Tb_Task.Status ='1'"
                    SearchOperator="UserDefine" />
                <jasp:ParameterItem ParaType="String" ParaName="xx" DataName="BigKind ='0'" SearchOperator="UserDefine" />
                        ...
                        <ItemTemplate>
                            <tr class="text-c">
                                ...
                                <td class="operate">
                                    <% if (this.IsManager)
                                       { %>
                                    <a onclick="javascript:return TuiTask('<%#Eval("GuidID") %>');" href='#' title="将工单退回，重新安排">
                                        <img alt="" src="/images/icon/office_140.png" width="16" height="16" border="0" />退单</a><br />
                                    <% } %>
                                    <span id="IsCanDo" runat="server"><a onclick="javascript:return SubmitCheckTask('<%#Eval("GuidID") %>');"
                                        href='#' title="提交工单审核">
                                        <img alt="" src="/images/gou2.gif" width="16" height="16" border="0" />提交</a><br />
                                    </span>
                                    <% if (this.IsManager)
                                       { %>
                                    <a onclick="javascript:return DelData('<%#Eval("GuidID") %>');" title="取消工单" href='#'>
    ...
    <script type="text/javascript">
        ...
        function DelData(dataid) {
            layer.confirm("提示：确定要【取消工单】吗？", function() {
                $.ajax({
                    type: 'POST',
                    ...
                    success: function(data) {
                        if ("ok" == data) {
                            layer.alert("【取消工单】操作成功！", function() {
                                $('#butSearch').click();  //重新加载页面
                            });
                        } else {
                            layer.alert("【取消工单】操作失败！");
                        }
                    }
                });
            });
        ...
        //4*--退单
        function TuiTask(dataid) {
            layer.confirm("提示：确定要【退单】吗？", function() {
                $.ajax({
                    type: 'POST',
                    url: "Handler/DoAction.aspx",
                    data: {
                        KeyID: dataid,
                        Action: 'TuiTask_Tb_Task'
                    },
                    success: function(data) {
                        if ("ok" == data) {
                            layer.alert("【退单】操作成功！", function() {
                                $('#butSearch').click();  //重新加载页面
                            });
                        } else {
                            layer.alert("【退单】操作失败！");
        ...
        //5*--提交工单审核
        function SubmitCheckTask(dataid) {
            layer.confirm("提示：确定要【提交工单审核】吗？", function() {
                $.ajax({
                    type: 'POST',
                    url: "Handler/DoAction.aspx",
                    data: {
                        KeyID: dataid,
                        Action: 'SubmitCheck_Tb_Task'
                    },
                    success: function(data) {
                        if ("ok" == data) {
                            layer.alert("【提交工单审核】操作成功！", function() {
                                $('#butSearch').click();  //重新加载页面
                            });
                        } else {
                            layer.alert("【提交工单审核】操作失败！");
                        }
                    }
                });
            });

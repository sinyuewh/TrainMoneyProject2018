<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Tb_Task_List0.aspx.cs"
    MasterPageFile="~/SysManager/include/H-uiSecond.Master" Inherits="OAWebSite2016.SysManager.WebUI.WorkPlanMng.Tb_Task_List0" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <!--导航区设置-->
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 工单管理 <span class="c-gray en">&gt;</span> 待派工工单 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="pd-20">
        <!--查询区设置-->
         <div class="text-c">
                    ...
                    <asp:Repeater ID="Repeater1" runat="server" DataSourceID="data1" EnableViewState="false">
                        <ItemTemplate>
                            <tr class="text-c">
                                ...
                                <td class="operate">
                                    <a onclick="javascript:return JieTaskData('<%#Eval("GuidID") %>');" href='#' title="接单，每个用户最多只能接三个工单">
                                        <img alt="" src="/images/gou2.gif" width="16" height="16" border="0" />接单</a><br />
                                    <% if (this.IsManager)
                                       { %>
                                    <a onclick="javascript:return DelData('<%#Eval("GuidID") %>');" href='#' title="取消工单">
    ...
    <script type="text/javascript">
        ...
        function NewData() {
            var title = "添加工单";
            var url = "Tb_Task_Detail_Add.aspx?parentGuid=" + $('#<%=parentGuid.ClientID %>').val();
            layer_show(title, url, 900, 650);
        }

        //导入Excel文件
        function ImportExcelData() {
            var title = "利用Excel导入工单";
            var url = "Tb_Task_ImportExcel.aspx?parentGuid=" + $('#<%=parentGuid.ClientID %>').val();
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if ("ok" == data) {
                                ...
                                layer.alert("【接单】操作失败，可能的原因是你已有三个未完成的工单！");
                            }
                        }
                    });
                });
